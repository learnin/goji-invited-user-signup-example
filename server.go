...
func showSignupPage(c web.C, w http.ResponseWriter, r *http.Request) {
...
func renderSignupPage(c web.C, w http.ResponseWriter, r *http.Request, form Form) {
...
func execute(c web.C, w http.ResponseWriter, r *http.Request) {
	var form Form
	...
	fmt.Println(hash(form.UserId))
	if hash(form.UserId) != form.HashKey {
		form.Msg = "ユーザーIDを正しく入力してください。"
		renderSignupPage(c, w, r, form)
		return
	}
	alreadyExist, err := existsUser()
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
func addUser(l *ldap.LDAPConnection) error {
...
func main() {
	goji.Get("/signup/:hashKey", showSignupPage)
	goji.Post("/signup/execute", execute)
	goji.Get("/signup/complete", showCompletePage)
	goji.Serve()
}
